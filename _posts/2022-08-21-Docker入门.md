...
```
2. 执行`docker build -t nginx:lsd01 .`, 后面带 `·` 表示`DockerFile`在当前目录下，`:`后面表示tag，没有则默认 `latest`
3. 在客户端执行 `docker run -d -p 9999:80 nginx:lsd01`

## 5.2 DockerFile里常用指令
**FROM**：定制的镜像都是基于 `FROM` 的镜像，这里的 `nginx` 就是定制需要的基础镜像。后续的操作都是基于 `nginx`。

**RUN**：用于执行后面跟着的命令行命令，一个 `run` 会在 `build` 时新建一层，一般多个 run 会通过 `&&` 进行合并。有以下两种格式：
...
## 作业
使用`Docker`，将我们正在使用项目搭建起来并能正常访问和功能正常使用。需要注意，不允许将镜像上传到仓库
