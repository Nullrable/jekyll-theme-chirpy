...
Docker能兴起的原因，Docker 镜像是直接由一个完整操作系统的所有文件和目录构成的，所以这个压缩包里的内容跟你本地开发和测试环境用的操作系统是完全一样的。
，它解决了应用打包和发布这一困扰运维人员多年的技术难题；而另一方面，就是因为它第一次把一个纯后端的技术概念，通过非常友好的设计和封装，交到了最广大的开发者群体手里。

# 2. 技术基础
基于Linux的两个特性Namespace，Linux Control Group，提供了可见性和限制能力
...
```
2. 执行`docker build -t nginx:lsd01 .`, 后面带 `·` 表示DockerFile在当前目录下，`:`后面表示tag，没有则默认 `latest`
...
## 作业
使用docker，将我们正在使用项目搭建起来并能正常访问和功能正常使用。需要注意，不允许将镜像上传到仓库
